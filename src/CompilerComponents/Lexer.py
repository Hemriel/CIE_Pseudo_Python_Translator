...
def tokenize_line(line: CleanLine) -> list[Token]:
    """Tokenize one line of code

    Args:
        line (clean_line): Line to tokenize, doesn't support multiline string literals

    Raises:
        ValueError: too many decimal points in number
        ValueError: invalid date format
        ValueError: invalid character literal
        ValueError: unterminated string literal
        ValueError: unknown operator
        ValueError: unknown character

    Returns:
        list[Token]: List of tokens found in the line
    """
    ...
    while i < len(line.content):
        ...
        if i >= len(line.content):
            break
        token, _start, _end, next_i, _kind, _lexeme = _scan_next_token(line, i)
        tokens.append(token)
        i = next_i
    return tokens


def make_symbol_table(tokens: list[Token]) -> SymbolTable:
    table = SymbolTable()
    scopes = ["global"]
    for i, token in enumerate(tokens):
        ...
        if token.type == TokenType.STATEMENT_KEYWORDS and token.value in [
            ...
            # Look ahead to find the identifier being declared
            if next_token and next_token.type == TokenType.IDENTIFIER:
                if token.value == "CONSTANT":
                    table.declare(
                        next_token.value,
                        token.line_number,
                        constant=True,
                ...
                elif token.value in ["FUNCTION", "PROCEDURE"]:
                    table.declare(
                        next_token.value,
                        token.line_number,
                        data_type=token.value.lower(),
                        scope=scopes[-1],
                    )
                    scopes.append(next_token.value)  # Enter new scope
                    ...
                    scopes.append(next_token.value)  # Enter type scope
                else:
                    table.declare(next_token.value, token.line_number, scope=scopes[-1])
        elif token.type == TokenType.STATEMENT_KEYWORDS and token.value in [
            "ENDFUNCTION",
            "ENDPROCEDURE",
            "ENDTYPE",
        ]:
...
def lexical_analysis(lines: list[str], filename="temp") -> list[Token]:
    """
    Performs lexical analysis on a list of lines of code. Writes tokens to a file, as well as error messages if any.

    Args:
        lines (list[str]): List of lines of code as strings.
        filename (str, optional): Base filename for output token file. Defaults to "temp". File will be named <filename>_tokens.txt.

    Returns:
        tuple[list[Token], dict]: List of tokens found in the code, including an EOF token at the end, and a preliminary symbol table.
    """

    # Step 1: Clean comments and blank lines
    cleaned_lines = []
    for i, line in enumerate(remove_comments(lines)):
        if not is_blank_line(line):
            cleaned_lines.append(CleanLine(line, i + 1))

    # Step 2: Tokenize lines
    all_tokens = []
    ...
    with open(f"{filename}_tokens.txt", "w") as f:
        try:
            for cl in cleaned_lines:
                ...
                for token in tokens:
                    f.write(
                        f"    {str(token.type).replace('TokenType.', '')}: {token.value}\n"
                    )
        ...
        all_tokens.append(
            Token(
                ...
                "END_OF_FILE",
                cleaned_lines[-1].line_number if cleaned_lines else 0,
            )
        )
        f.write("    END_OF_FILE: \n")

    # Step 3: Create symbol table
    symbol_table = make_symbol_table(all_tokens)

    # Write preliminary symbol table to file
    with open(f"{filename}_symbol_table.md", "w") as f:
        f.write(symbol_table.to_markdown())

    print(
        f"Lexical analysis completed successfully. Tokens written to {filename}_tokens.txt"
    )
    print(f"Symbol table written to {filename}_symbol_table.md")
    return all_tokens
